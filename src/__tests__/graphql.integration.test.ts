...
import { constructTestServer, createMockOptionWithReturnValue } from 'tests/_utils'
...
describe('Queries', () => {
  ...
  xit('fetches ComicBook', async () => {
    const mockComicBook = { _id: '1', title: 'Comic', url: '/path', releaseDate: '', issue: '#12', coverUrl: '/' }
    const { server, comicBookAPI } = constructTestServer()
    comicBookAPI.getById = jest.fn().mockReturnValueOnce(createMockOptionWithReturnValue(mockComicBook))
